...
extends EventEmitter2 {
	...
	public Agent(NodeContext ctx, ReqOptions options) throws Exception {
		...
		self.sockets = new Hashtable<String, List<TCP.Socket>>();
		self.freeSockets = new Hashtable<String, List<TCP.Socket>>();
			...
			public void onEvent(Object raw) throws Exception {
				...
				if (!socket.isDestroyed() &&
					...
					if (req!=null &&
						...
						List<TCP.Socket> freeSockets = self.freeSockets.containsKey(name) ? self.freeSockets.get(name) : null;
	...
	public void addRequest(ClientRequest req, ReqOptions options) throws Exception {
	  ...
	  if (freeLen > 0) {
	    ...
	    TCP.Socket socket = this.freeSockets.get(name).remove(0);
	...
	public TCP.Socket createSocket(ClientRequest req, final ReqOptions options) throws Exception {
		...
		final TCP.Socket s = TCP.createConnection(
				context, 
				options.servername, 
				options.port, 
	...
	public void destroy() throws Exception {
	  ...
	  sets.forEach(function(set) {
		...
		for (List<TCP.Socket> ss : this.freeSockets.values())
			for (TCP.Socket s : ss)
				s.destroy(null);
		
		for (List<TCP.Socket> ss : this.sockets.values())
			for (TCP.Socket s : ss)
				s.destroy(null);
	}
